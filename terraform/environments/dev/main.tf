...
locals {
  ...
  # GitHub Actions OIDC - application deploy (restrict to repos + branch)
  github_app_deploy_sub_patterns = [
    for repo in var.github_app_repos :
    "repo:${var.github_org}/${repo}:ref:refs/heads/${var.github_app_deploy_branch}"
  ]
}
...
resource "aws_iam_role" "github_actions_apps_deploy" {
  name = var.github_actions_apps_deploy_role_name
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "token.actions.githubusercontent.com:sub" = local.github_app_deploy_sub_patterns
          }
        }
      }
    ]
...
############################
# IAM Policies - GitHub Actions (Apps)
# - apps-deploy: ECS/ECR deploy (push image + update service)
############################

# apps-deploy: ECR push + ECS update + PassRole restricted to ECS roles
resource "aws_iam_role_policy" "github_actions_apps_deploy" {
  name = "${local.project_prefix}-apps-deploy"
  role = aws_iam_role.github_actions_apps_deploy.id
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          aws_ecr_repository.upload.arn,
          aws_ecr_repository.download.arn,
